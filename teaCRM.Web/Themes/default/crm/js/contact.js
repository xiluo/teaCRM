...
function InitGrid() {
    ...
    grid = $("#grid-data").bootgrid({
        ...
        rowCount: [10, 30, 50],
        templates: {
            header: "<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-12 actionBar\"><div class=\"btn-group\" style=\"float:left;\"><button class=\"btn btn-default tip\" title=\"添加联系人\" onclick=\" add_contact(" + $("#cus_id").val() + ") \"><span class=\"glyphicon glyphicon-plus\"></span>添加</button><button id=\"muti-del\" class=\"btn btn-default tip\" title=\"删除联系人\" ><span class=\"glyphicon glyphicon glyphicon glyphicon-trash\"></span>批量删除</button></div>" +
        ...
        formatters: {
            ...
            "commands": function(column, row) {
                return "<button type=\"button\"  class=\"btn btn-link btn-sm btn-cmd tip\" onclick=\"edit_contact(0," + row.id + ");\" title=\"修改联系人【" + row.con_name + "】\"><span class=\"glyphicon glyphicon-pencil\"></span></button>" +
...
/**
*添加联系人
*如果客户customer_id为空直接添加联系人，不为空，则添加当前客户对应的联系人
*@author terwer
...
function add_contact(customer_id) {
    ...
    showTopWindow(win_id, show_url, "添加联系人", 800, 360, function() {
        //调用iframe方法验证表单
...
/**
*修改联系人
*如果客户customer_id为空直接修改联系人，不为空，则修改当前客户对应的联系人
*@author terwer
*@version 1.0 2014/09/24 
*@param customer_id 客户id（可为空） 
*@return 无返回值
...
function edit_contact(customer_id,contact_id) {
    ...
    showTopWindow(win_id, show_url, "修改联系人", 800, 360, function () {
        //调用iframe方法验证表单
        var flag = window.top.document.getElementById("frm_" + win_id).contentWindow.form_valid();
        if (!flag) {
            return false;
...
function del(ids) {
    ...
    var del_tips = "确认删除该联系人吗？";
    try {
        if (ids.toString().indexOf(",") > 0) {
            var del_tips = "确认批量删除选中联系人吗？";
        }
    } catch (e) {
