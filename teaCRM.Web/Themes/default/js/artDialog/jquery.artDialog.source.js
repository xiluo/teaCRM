...
artDialog.fn = artDialog.prototype = {
	...
	size: function (width, height) {
			...
			if (typeof width === 'number') {
				...
				if (width === 'auto') {
					...
					if (pWidth === 0) {
						...
						wrapStyle.width = 'auto';
						//style.width = Math.max(that.config.minWidth, pWidth) + 'px';
		...
		if (height || height === '') {
			...
			if (typeof height === 'number') {
				...
				if (height === 'auto') {
					...
					var pHeight = parseInt(height) || 0;
					if (pHeight === 0) {
						style.height = pHeight;
					}
					else {
						//style.height = Math.max(that.config.minHeight, pHeight) + 'px';
		...
		if (scrollHeight) {
			...
			else if (typeof scrollHeight === 'string') {
				var pHeight = parseInt(scrollHeight) || 0;
				if (pHeight === 0) {
					contentStyle.height = pHeight;
					contentStyle.overflow = 'auto';
				}
				else {
					contentStyle.height = scrollHeight + 'px';
