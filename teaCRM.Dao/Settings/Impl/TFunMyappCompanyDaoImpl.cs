...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunMyappCompanyDao接口的Dao类。 2014-09-17 06:52:51 By 唐有炜
    /// </summary>
 public class TFunMyappCompanyDaoImpl:ITFunMyappCompanyDao
    {
	  ...
	  #region 读操作
       ...
       public List<TFunMyappCompany> GetList(Expression<Func<TFunMyappCompany, bool>> predicate)
        ...
        public IQueryable<Object> GetFields(string selector, string predicate)
        {
            using (teaCRMDBContext db=new teaCRMDBContext())
            {
                var model = db.TFunMyappCompanies.Where(predicate).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
	  ...
	  //===========================================================================
	   /// <summary>
        ...
        /// <returns>集合</returns>
        public IEnumerable<TFunMyappCompany> GetListBySql(string sql, dynamic namedParameters)
        {
          using (teaCRMDBContext db=new teaCRMDBContext())
            {
   ...
   #region 写操作
       ...
       /// <summary>
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpadateEntity(TFunMyappCompany entity)
        {
            using (teaCRMDBContext db=new teaCRMDBContext())
            {
