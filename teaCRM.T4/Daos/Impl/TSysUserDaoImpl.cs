...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITSysUserDao接口的Dao类。 2014-09-09 03:48:14 By 唐有炜
    /// </summary>
 public class TSysUserDaoImpl:ITSysUserDao
    {
       ...
       public   bool ExistsEntity(Expression<Func<TSysUser , bool>> predicate)
	   {
	      ...
	      //查询分页
        public IPagination<TSysUser> GetListByPage(int pageIndex, int pageSize, int rowCount,
            Expression<Func<TSysUser, bool>> predicate)
        {
            using (teaCRMDBContext db = new teaCRMDBContext())
            {
                var models = db.TSysUsers.Where(predicate).ToPagination(pageIndex, pageSize, rowCount);
                return models;
            }
        }
