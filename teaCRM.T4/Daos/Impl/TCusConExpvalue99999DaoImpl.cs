...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITCusConExpvalue99999Dao接口的Dao类。 2014-09-16 08:42:54 By 唐有炜
    /// </summary>
 public class TCusConExpvalue99999DaoImpl:ITCusConExpvalue99999Dao
    {
	  ...
	  #region 读操作
	   ...
	   /// <summary>
        ...
        /// <returns>返回所有数据总数</returns>
       public int GetCount(Expression<Func<TCusConExpvalue99999, bool>> predicate)
        {
             using (teaCRMDBContext db=new teaCRMDBContext())
            {
	   ...
	   //查询分页
        public IPagination<TCusConExpvalue99999> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, teaCRM.Entity.teaCRMEnums.OrderEmum> orders,
            Expression<Func<TCusConExpvalue99999, bool>> predicate)
        {
            using (teaCRMDBContext db = new teaCRMDBContext())
            {
                var roles = db.TCusConExpvalue99999s.Where(predicate);
