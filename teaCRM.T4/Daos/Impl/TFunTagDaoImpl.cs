...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunTagDao接口的Dao类。 2014-09-09 03:48:14 By 唐有炜
    /// </summary>
 public class TFunTagDaoImpl:ITFunTagDao
    {
       ...
       public List<TFunTag> GetList(Expression<Func<TFunTag, bool>> predicate)
        ...
        public IQueryable<Object> GetFields(string selector, string predicate)
        {
            using (teaCRMDBContext db = new teaCRMDBContext())
            {
                var model = db.TFunTags.Where(predicate).Select(selector);
                var sqlText=model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(Expression<Func<TFunTag , bool>> predicate)
	   {
	      ...
	      //查询分页
        public IPagination<TFunTag> GetListByPage(int pageIndex, int pageSize, int rowCount,
            Expression<Func<TFunTag, bool>> predicate)
        {
            using (teaCRMDBContext db = new teaCRMDBContext())
            {
                var models = db.TFunTags.Where(predicate).ToPagination(pageIndex, pageSize, rowCount);
                return models;
            }
        }
