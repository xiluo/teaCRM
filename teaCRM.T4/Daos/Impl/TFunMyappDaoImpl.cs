...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunMyappDao接口的Dao类。 2014-09-05 05:34:54 By 唐有炜
    /// </summary>
 public class TFunMyappDaoImpl:ITFunMyappDao
    {
        ...
        public List<TFunMyapp> GetList() 
        {
          using (teaCRMDBContext db=new teaCRMDBContext())
            {
             var models= db.TFunMyapps.ToList();
			 return models;
            }
        }
       ...
       public List<TFunMyapp> GetList(Expression<Func<TFunMyapp, bool>> predicate)
        {
             using (teaCRMDBContext db=new teaCRMDBContext())
            {
             var models= db.TFunMyapps.Where<TFunMyapp>(predicate).ToList();
			 return models;
            }
        }
        ...
        public TFunMyapp GetEntity(Expression<Func<TFunMyapp, bool>> predicate) 
        {
            using (teaCRMDBContext db=new teaCRMDBContext())
            {
                var model =db.TFunMyapps.Where<TFunMyapp>(predicate).SingleOrDefault();
                return model;
		    }
        }
       ...
       public   bool ExistsEntity(Expression<Func<TFunMyapp , bool>> predicate)
	   {
		 ...
		 //查询分页
    public  List<TFunMyapp> GetListByPage(int pageIndex, int pageSize, Expression<Func<TFunMyapp , bool>> predicate)
	  {
	   using (teaCRMDBContext db=new teaCRMDBContext())
            {
             var models= db.TFunMyapps.ToList();
