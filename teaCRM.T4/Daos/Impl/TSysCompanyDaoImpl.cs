...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITSysCompanyDao接口的Dao类。 2014-09-05 05:34:54 By 唐有炜
    /// </summary>
 public class TSysCompanyDaoImpl:ITSysCompanyDao
    {
       ...
       public List<TSysCompany> GetList(Expression<Func<TSysCompany, bool>> predicate)
        {
             using (teaCRMDBContext db=new teaCRMDBContext())
            {
             var models= db.TSysCompanies.Where<TSysCompany>(predicate).ToList();
			 return models;
            }
        }
       ...
       public   bool ExistsEntity(Expression<Func<TSysCompany , bool>> predicate)
	   {
		 ...
		 //查询分页
    public  List<TSysCompany> GetListByPage(int pageIndex, int pageSize, Expression<Func<TSysCompany , bool>> predicate)
	  {
	   using (teaCRMDBContext db=new teaCRMDBContext())
            {
             var models= db.TSysCompanies.ToList();
