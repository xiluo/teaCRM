...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunOperatingDao接口的Dao类。 2014-09-10 07:35:30 By 唐有炜
    /// </summary>
 public class TFunOperatingDaoImpl:ITFunOperatingDao
    {
       ...
       public   bool ExistsEntity(Expression<Func<TFunOperating , bool>> predicate)
	   {
	      ...
	      //查询分页
        public IPagination<TFunOperating> GetListByPage(int pageIndex, int pageSize, int rowCount,
            Expression<Func<TFunOperating, bool>> predicate)
        {
            using (teaCRMDBContext db = new teaCRMDBContext())
            {
                var models = db.TFunOperatings.Where(predicate).ToPagination(pageIndex, pageSize, rowCount);
                return models;
            }
        }
