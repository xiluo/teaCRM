...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITSysRoleDao接口的Dao类。 2014-09-09 03:48:14 By 唐有炜
    /// </summary>
 public class TSysRoleDaoImpl:ITSysRoleDao
    {
       ...
       public List<TSysRole> GetList(Expression<Func<TSysRole, bool>> predicate)
        ...
        public IQueryable<Object> GetFields(string selector, string predicate)
        {
            using (teaCRMDBContext db = new teaCRMDBContext())
            {
                var model = db.TSysRoles.Where(predicate).Select(selector);
                var sqlText=model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
       ...
       public   bool ExistsEntity(Expression<Func<TSysRole , bool>> predicate)
	   {
            ...
            Expression<Func<TSysRole, bool>> predicate)
        {
            using (teaCRMDBContext db = new teaCRMDBContext())
            {
                var models = db.TSysRoles.Where(predicate).ToPagination(pageIndex, pageSize, rowCount);
                return models;
            }
        }
