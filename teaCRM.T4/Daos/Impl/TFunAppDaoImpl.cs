...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITFunAppDao接口的Dao类。 2014-09-05 05:34:54 By 唐有炜
    /// </summary>
 public class TFunAppDaoImpl:ITFunAppDao
    {
       ...
       public List<TFunApp> GetList(Expression<Func<TFunApp, bool>> predicate)
        {
             using (teaCRMDBContext db=new teaCRMDBContext())
            {
             var models= db.TFunApps.Where<TFunApp>(predicate).ToList();
			 return models;
            }
        }
        ...
        public TFunApp GetEntity(Expression<Func<TFunApp, bool>> predicate) 
        {
            using (teaCRMDBContext db=new teaCRMDBContext())
            {
                var model =db.TFunApps.Where<TFunApp>(predicate).SingleOrDefault();
                return model;
		    }
        }
       ...
       public   bool ExistsEntity(Expression<Func<TFunApp , bool>> predicate)
	   {
		 ...
		 //查询分页
    public  List<TFunApp> GetListByPage(int pageIndex, int pageSize, Expression<Func<TFunApp , bool>> predicate)
	  {
	   using (teaCRMDBContext db=new teaCRMDBContext())
            {
             var models= db.TFunApps.ToList();
