...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITCusExpvalue99999Dao接口的Dao类。 2014-09-10 07:35:30 By 唐有炜
    /// </summary>
 public class TCusExpvalue99999DaoImpl:ITCusExpvalue99999Dao
    {
       ...
       public   bool ExistsEntity(Expression<Func<TCusExpvalue99999 , bool>> predicate)
	   {
	      ...
	      //查询分页
        public IPagination<TCusExpvalue99999> GetListByPage(int pageIndex, int pageSize, int rowCount,
            Expression<Func<TCusExpvalue99999, bool>> predicate)
        {
            using (teaCRMDBContext db = new teaCRMDBContext())
            {
                var models = db.TCusExpvalue99999s.Where(predicate).ToPagination(pageIndex, pageSize, rowCount);
                return models;
            }
        }
