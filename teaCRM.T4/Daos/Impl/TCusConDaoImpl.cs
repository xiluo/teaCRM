...
namespace  teaCRM.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITCusConDao接口的Dao类。 2014-09-09 03:48:14 By 唐有炜
    /// </summary>
 public class TCusConDaoImpl:ITCusConDao
    {
       ...
       public   bool ExistsEntity(Expression<Func<TCusCon , bool>> predicate)
	   {
	      ...
	      //查询分页
        public IPagination<TCusCon> GetListByPage(int pageIndex, int pageSize, int rowCount,
            Expression<Func<TCusCon, bool>> predicate)
        {
            using (teaCRMDBContext db = new teaCRMDBContext())
            {
                var models = db.TCusCons.Where(predicate).ToPagination(pageIndex, pageSize, rowCount);
                return models;
            }
        }
